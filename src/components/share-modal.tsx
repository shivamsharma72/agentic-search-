...
export default function ShareModal({ open, onOpenChange, marketTitle, verdict, confidence }: ShareModalProps) {
  ...
  const shareUrl = `https://omnisense.xyz?via=share&r=${Math.random().toString(36).substr(2, 9)}`;
...
AI-powered deep research + analyst-grade report in 5s. Try it: ${shareUrl}`;
  ...
  const redditBody = `Just analyzed this prediction market on OmniSense and got a ${confidence}% confidence ${verdict} verdict.
