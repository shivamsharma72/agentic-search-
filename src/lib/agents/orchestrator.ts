...
export interface PolymarketOrchestratorOpts {
  ...
  // Fast Mode options
  skipCritic?: boolean; // Skip critic + follow-up research
  maxSearches?: number; // Limit searches per cycle
  useParallelResearch?: boolean; // Execute research calls in parallel
  useFastLLM?: boolean; // Use faster LLM (Haiku) for extraction
}
...
export interface UnifiedOrchestratorOpts {
  ...
  sessionId?: string; // Session ID for cost tracking
  customerId?: string; // Polar customer ID for LLM tracking
  // Optimization: Always run in fast mode (parallel research, skip critic/follow-up)
}

export async function runPolymarketForecastPipeline(
  opts: PolymarketOrchestratorOpts
): Promise<ForecastCard> {
  ...
  const optimalInterval =
    opts.historyInterval || selectOptimalHistoryInterval(firstFetch);
  const autoDrivers =
    ...
    optimalInterval === "1d"
      ...
      : await (async () => {
          ...
          onProgress?.("complete_data_ready", {
            ...
            outcomes: Object.keys(refreshed.market_facts.token_map ?? {})
              .length,
            ...
            eventSummary: (refreshed as any).event_summary
              ? {
                  isMultiCandidate: (refreshed as any).event_summary
                    .is_multi_candidate,
                  totalMarkets: (refreshed as any).event_summary.total_markets,
                  activeMarkets: (refreshed as any).event_summary
                    .active_markets,
                  topCandidates: (
                    refreshed as any
                  ).event_summary.top_candidates?.map((c: any) => ({
                    name: c.name,
                    implied_probability: c.implied_probability,
                    volume: c.volume,
                    liquidity: c.liquidity,
  ...
  console.log(
    `📥 Market data ready: interval=${optimalInterval}, withBooks=${
      opts.withBooks ?? true
    }, withTrades=${opts.withTrades ?? false}`
  );
  ...
  onProgress?.("researching", {
    ...
    searchSeeds: plan.searchSeeds.length,
    fastMode: opts.useParallelResearch ? "parallel" : "sequential",
    maxSearches: opts.maxSearches,
  });

  console.log(`🔍 === RESEARCH CYCLE 1: Initial Evidence Gathering ===`);
  console.log(
    `⚡ Fast Mode: parallel=${opts.useParallelResearch}, maxSearches=${
      opts.maxSearches || "unlimited"
    }`
  );

  // Execute research in parallel if Fast Mode enabled
  let initialPro: Evidence[], initialCon: Evidence[], adjacent: Evidence[];

  if (opts.useParallelResearch) {
    console.log(`⚡ Running research in PARALLEL mode...`);
    const [bothSides, adjacentResults] = await Promise.all([
      researchBothSides(question, plan as any, marketData, opts.sessionId),
      conductAdjacentResearch(
        question,
        plan as any,
        marketData,
    ...
    initialPro = bothSides.pro;
    initialCon = bothSides.con;
    adjacent = adjacentResults;
  } else {
    console.log(`🐌 Running research in SEQUENTIAL mode...`);
    const bothSides = await researchBothSides(
      question,
      plan as any,
      marketData,
      opts.sessionId
    ...
    initialPro = bothSides.pro;
    initialCon = bothSides.con;
    adjacent = await conductAdjacentResearch(
      question,
      plan as any,
      marketData,
      opts.sessionId
  ...
  console.log(
    `🔎 Initial research complete: pro=${initialPro.length}, con=${initialCon.length}`
  );
  console.log(
    `🔎 Adjacent signals: pro=${adjacentPro.length}, con=${adjacentCon.length}`
  );
  ...
  onProgress?.("initial_research_complete", {
    ...
    response: {
      proEvidence: [...initialPro, ...adjacentPro].map((e) => ({
        claim: e.claim,
        type: e.type,
        polarity: e.polarity,
        urls: e.urls,
      ...
      conEvidence: [...initialCon, ...adjacentCon].map((e) => ({
        claim: e.claim,
        type: e.type,
        polarity: e.polarity,
        urls: e.urls,
  ...
  // Step 6: CRITIC ANALYSIS - Identify gaps and provide feedback (Skip in Fast Mode)
  ...
  if (opts.skipCritic) {
    console.log(`⚡ === CRITIC ANALYSIS: SKIPPED (Fast Mode) ===`);
    onProgress?.("criticism_skipped", {
      message: "Critical analysis skipped - Fast Mode enabled",
    ...
    critique = await criticAgent(question, initialPro, initialCon);
    ...
    console.log(
      `   - Follow-up searches suggested: ${critique.followUpSearches.length}`
    );
    console.log(
      `   - Correlation adjustments: ${
        Object.keys(critique.correlationAdjustments).length
      }`
    );
  ...
  // Step 7: SECOND RESEARCH CYCLE - Targeted follow-up research (Skip in Fast Mode)
  if (!opts.skipCritic && critique && critique.followUpSearches.length > 0) {
    ...
    const followUpResults = await conductFollowUpResearch(
      question,
      critique.followUpSearches,
      marketData,
      opts.sessionId
    );
    ...
    console.log(
      `🔎 Follow-up research complete: +${followUpResults.pro.length} pro, +${followUpResults.con.length} con, +${followUpResults.neutral.length} neutral`
    );
    console.log(
      `🔎 Total evidence: pro=${finalPro.length}, con=${finalCon.length}, neutral=${neutralEvidence.length}`
    );
    ...
    onProgress?.("followup_research_complete", {
      ...
      response: {
        additionalProEvidence: followUpResults.pro.map((e) => ({
          claim: e.claim,
          type: e.type,
          polarity: e.polarity,
          urls: e.urls,
        })),
        additionalConEvidence: followUpResults.con.map((e) => ({
          claim: e.claim,
          type: e.type,
          polarity: e.polarity,
          urls: e.urls,
        })),
        neutralEvidence: followUpResults.neutral.map((e) => ({
          claim: e.claim,
          type: e.type,
          urls: e.urls,
        })),
        totalProEvidence: finalPro.map((e) => ({
          claim: e.claim,
          type: e.type,
          polarity: e.polarity,
          urls: e.urls,
        })),
        totalConEvidence: finalCon.map((e) => ({
          claim: e.claim,
          type: e.type,
          polarity: e.polarity,
          urls: e.urls,
  ...
  } else if (opts.skipCritic) {
    console.log(`⚡ === RESEARCH CYCLE 2: SKIPPED (Fast Mode) ===`);
    onProgress?.("followup_research_skipped", {
      message: "Follow-up research skipped - Fast Mode enabled",
  ...
  function dedupeByUrl(items: Evidence[]): Evidence[] {
    ...
    for (const ev of items) {
      ...
      if (unique.length) {
        try {
          originHost = new URL(unique[0]).hostname
            .toLowerCase()
            .replace(/^www\./, "");
        } catch {}
      }
  ...
  const allEvidence: Evidence[] = dedupeByUrl([
    ...finalPro,
    ...finalCon,
    ...neutralEvidence,
  ]);
  ...
  const { pNeutral, pAware, influence, clusters, filteredEvidence } =
    await analystAgentWithCritique(
      question,
      p0,
      allEvidence,
      critique,
  ...
  console.log(
    `🧮 Enhanced aggregation complete: pNeutral=${pNeutral.toFixed(
      3
    )}, pAware=${
      pAware !== undefined ? pAware.toFixed(3) : "n/a"
    }, influenceItems=${influence.length}, clusters=${clusters.length}`
  );
  console.log(
    `🧮 Evidence used in final analysis: ${
      filteredEvidence?.length || allEvidence.length
    }/${allEvidence.length} (${
      allEvidence.length - (filteredEvidence?.length || allEvidence.length)
    } filtered out)`
  );
  ...
  onProgress?.("aggregation_complete", {
    ...
    evidenceFiltered:
      allEvidence.length - (filteredEvidence?.length || allEvidence.length),
    topInfluences: influence.slice(0, 5).map((i) => ({
    ...
    response: {
      ...
      evidenceFiltered: filteredEvidence
        ? allEvidence.filter(
            (e) => !filteredEvidence.some((fe) => fe.id === e.id)
          )
        : [],
    },
  });
  ...
  const markdown = await reporterAgent(
    question,
    p0,
    pNeutral,
    pAware,
  ...
  console.log(
    `⏱️  Total pipeline time: ${((Date.now() - t0) / 1000).toFixed(1)}s`
  );
 ...
 */
export async function runUnifiedForecastPipeline(
  opts: UnifiedOrchestratorOpts
): Promise<ForecastCard> {
  ...
  onProgress?.("complete_data_ready", {
    ...
    eventSummary: firstFetch.event_summary
      ? {
          ...
          topCandidates: firstFetch.event_summary.top_candidates?.map(
            (c: any) => ({
              name: c.name,
              implied_probability: c.implied_probability,
              volume: c.volume,
          ...
          onProgress?.("complete_data_ready", {
            ...
            eventSummary: refreshed.event_summary
              ? {
                  ...
                  topCandidates: refreshed.event_summary.top_candidates?.map(
                    (c: any) => ({
                      name: c.name,
                      implied_probability: c.implied_probability,
                      volume: c.volume,
  ...
  console.log(
    `📥 Market data ready: platform=${platform}, interval=${optimalInterval}, withBooks=${
      opts.withBooks ?? true
    }, withTrades=${opts.withTrades ?? false}`
  );
  ...
  // Step 5: RESEARCH CYCLE - Evidence gathering (always parallel, optimized)
  onProgress?.("researching", {
    message: "Starting evidence research in parallel...",
    searchSeeds: plan.searchSeeds.length,
    mode: "optimized-parallel",
  });

  console.log(`🔍 === RESEARCH CYCLE: Evidence Gathering (Optimized) ===`);
  console.log(`⚡ Running in PARALLEL mode (hardcoded optimization)`);

  // ALWAYS execute research in parallel (optimization hardcoded)
  const [bothSides, adjacent] = await Promise.all([
    researchBothSides(question, plan as any, marketData, opts.sessionId),
    conductAdjacentResearch(question, plan as any, marketData, opts.sessionId),
  ]);

  const initialPro = bothSides.pro;
  const initialCon = bothSides.con;

  const adjacentPro = adjacent.filter((e) => e.polarity > 0);
  const adjacentCon = adjacent.filter((e) => e.polarity < 0);
  console.log(
    `🔎 Research complete: pro=${initialPro.length}, con=${initialCon.length}`
  );
  console.log(
    `🔎 Adjacent signals: pro=${adjacentPro.length}, con=${adjacentCon.length}`
  );
  ...
  // Optimization: Critic analysis and follow-up research REMOVED
  // This saves ~90 seconds per analysis
  const finalPro = [...initialPro, ...adjacentPro];
  const finalCon = [...initialCon, ...adjacentCon];
  const neutralEvidence: Evidence[] = [];
  const critique: any = null; // No critic in optimized mode

  console.log(`⚡ === CRITIC & FOLLOW-UP: SKIPPED (Optimized Mode) ===`);
  console.log(`⚡ Time saved: ~90 seconds`);

  onProgress?.("optimization_applied", {
    message: "Optimized mode: Skipped critic analysis and follow-up research",
  ...
  // Step 9: Build both concise and detailed reports
  onProgress?.("reporting", {
    message: "Generating final reports...",
    drivers: autoDrivers,
  });

  // Generate both versions in parallel for speed
  const [conciseMarkdown, detailedMarkdown] = await Promise.all([
    conciseReporterAgent(
      question,
      p0,
      pNeutral,
      pAware,
  ...
  console.log(
    `📝 Reports generated: concise=${conciseMarkdown.length} chars, detailed=${detailedMarkdown.length} chars`
  );
  console.log(
    `⏱️  Total pipeline time: ${((Date.now() - t0) / 1000).toFixed(1)}s`
  );
  ...
  onProgress?.("report_complete", {
    message: "Final reports generated",
    conciseLength: conciseMarkdown.length,
    detailedLength: detailedMarkdown.length,
    totalTime: ((Date.now() - t0) / 1000).toFixed(1) + "s",
    response: {
      conciseReport: conciseMarkdown,
      detailedReport: detailedMarkdown,
      totalTimeSeconds: (Date.now() - t0) / 1000,
      finalProbabilities: {
        pNeutral: pNeutral,
        pAware: pAware,
  ...
  return makeForecastCard({
    ...
    markdownReport: conciseMarkdown, // Default to concise for display
    detailedReport: detailedMarkdown, // Include detailed for download/toggle
  });
}
